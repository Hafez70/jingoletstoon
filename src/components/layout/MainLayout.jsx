...
import {useNavigationMenu} from "configs/navigation";
//import { I18NameSpaces } from 'configs/i18n/langNameSpaces';
        ...
        <>
            {navigation.map((item, index) => (
                <NavLink
                    ...
                    to={item.to}
                    className={(isActive)=> {
                        let classes = 'text-gray-300 hover:bg-gray-700 hover:text-white group flex items-center px-2 py-2 text-base font-medium rounded-md '
                        return classes + (isActive ? 'bg-gray-900 text-white' : '')
                    }}
                >
                    <item.icon
...
const Logo = () => {
    //const { t } = useTranslation(I18NameSpaces.shared);
    return (
        <Link className="flex items-center text-white" to=".">
            {/*<img className="h-8 w-auto" src={logo} alt="Workflow" />*/}
            <span className="text-xl text-white font-semibold">appName</span>
        </Link>
    );
};
